...
def split_data(all_data, fold):
    ...
    # of each label in the entire dataset.
        ...
        test_super_set = [val for val in labeled_data[label] if val not in train_set]
        test_set = add_to_set(num_test_label, test_super_set, test_set)
...
def gen_time_split_data(all_data, time_point):
    
    new_allData = []
    
    if(is_valid_dataset(all_data)):
        max_time = get_max_time(all_data)
        assert(0 <= time_point and time_point < max_time)
        
        for (label, time, patient, ts) in all_data:
            nrows,ncols = ts.shape
            if time[0] <=time_point:
                B = np.array([copy.deepcopy(ts[0,:])])
                new_timePoints = [time[0]]
            else:
                continue
    
            for j in xrange(1,len(time)):
                if time[j]<=time_point:
                    if j>=len(ts):
                        print label, time, patient, ts 
                    row = np.array([ts[j,:]])
                    B = np.concatenate((B,row),axis=0)
                    new_timePoints.append(time[j])
                else:
                    break    
            new_allData.append((label,new_timePoints,patient,B))
...
#            for j in xrange(time_point,max_time):
#                if(j in time):
...
def test_gen_train_test_sets():
    all_data = [("good",[0,1], "0", [1,2]),
                ...
                ("bad",[4,6], "9", [1,5])]
    print(type(all_data))
    dat = gen_time_split_data(all_data, 2)
